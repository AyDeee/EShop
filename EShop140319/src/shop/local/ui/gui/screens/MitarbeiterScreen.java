...
import shop.local.ui.gui.BestandsGraphFrame;
...
public class MitarbeiterScreen extends Screen {
	...
	private void artikelEinfuegen() throws ArtikelExistiertBereitsException, FalscheBestandsgroesseException {
		...
		if (!nummer.isEmpty() && !bezeichnung.isEmpty() && !bestand.isEmpty() && !preis.isEmpty()) {
			...
			if (packungAlsInt > 1) {
				artikel = shop.fuegeMassengutArtikelEin(bezeichnung, nummerAlsInt, bestandAlsInt, preisAlsFloat,
						packungAlsInt, eingeloggterMitarbeiter);
				// Am Ende Listener, d.h. unseren Frame benachrichtigen:
				gui.onArtikelAdded();
			} else if (packungAlsInt == 1) {
				artikel = shop.fuegeArtikelEin(bezeichnung, nummerAlsInt, bestandAlsInt, preisAlsFloat,
						eingeloggterMitarbeiter);
				// Am Ende Listener, d.h. unseren Frame benachrichtigen:
				gui.onArtikelAdded();
			}
